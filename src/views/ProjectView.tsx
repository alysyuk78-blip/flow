    ...
    <div className="flex h-full flex-col">
      <div className="border-b border-gray-200 px-4 pt-4 xs:px-6 xs:pt-5 dark:border-gray-800">
        <div className="flex flex-wrap items-center gap-2 xs:gap-3">
          ...
          <div className="flex w-full items-center gap-2 text-sm text-gray-500 xs:ml-auto xs:w-auto">
            <span className="shrink-0">
              {done}/{all.length} готово
            </span>
            <div className="h-2 min-w-0 flex-1 overflow-hidden rounded-full bg-gray-200 xs:w-24 xs:flex-none dark:bg-gray-700">
              <div
        ...
        <div className="no-scrollbar mt-3 flex gap-1 overflow-x-auto xs:mt-4">
          {VIEWS.map((v) => (
            <button
              key={v.id}
              onClick={() => setView(v.id)}
              className={clsx(
                "touch-target flex shrink-0 items-center gap-1.5 rounded-t-lg border-b-2 px-3 py-2 text-sm transition-all duration-200 ease-smooth",
                view === v.id
